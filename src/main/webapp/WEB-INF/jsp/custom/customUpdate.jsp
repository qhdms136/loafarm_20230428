...
<div class="d-flex justify-content-center">
	<div class="cc-write-content">
		<div class="cc-write-title">커스터마이징 글수정</div>
		<div class="d-flex justify-content-center">
			<div class="cc-write-box">
				<div class="d-flex">
					<input type="text" id="subject" class="custom-subject form-control" placeholder="제목을 입력해주세요" value="${customPost.subject}">
				</div>
				...
				<div class="cc-file d-flex justify-content-between">
					...
					<div class="cc-btn-box d-flex">
						<button type="button" class="cc-clear btn-sm btn-outline-dark" id="customDeleteBtn" data-post-id="${customPost.id}">삭제하기</button>
						<button type="button" class="cc-save mx-3 btn-sm btn-secondary" id="customSaveBtn" data-post-id="${customPost.id}">저장하기</button>
						<button type="button" class="cc-list btn-sm" id="customListBtn">목록</button>
					</div>
				</div>
				<c:if test="${not empty customPost.imagePath}">
					<img class="cu-img-box" src="${customPost.imagePath}" alt="업로드 된 이미지" width="200">
				</c:if>	
			</div>
		</div>
	</div>
...
$(document).ready(function(){
	...
	$('#customDeleteBtn').on('click', function(){
		...
		$.ajax({
			...
			type:"DELETE"
			, url:"/custom/delete"
			, data:{"customPostId":customPostId}
			// response
			,success:function(data){
				if(data.code == 1){
					alert("삭제되었습니다.");
