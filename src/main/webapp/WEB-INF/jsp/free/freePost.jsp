		...
		<div class="d-flex justify-content-center">
			<div>
				<c:choose>
			        ...
			        <c:otherwise>
			            <a class="paging-link-ko" href="/free/free_list_view?category=${category}&page=${paging.page-1}&recommendCount=${recommendCount}">[이전]</a>
			        </c:otherwise>
			    </c:choose>
			    ...
			    <c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i" step="1">
			        <c:choose>
			            ...
			            <c:otherwise>
			                <a class="paging-link-number" href="/free/free_list_view?category=${category}&page=${i}&recommendCount=${recommendCount}"> &nbsp; ${i} &nbsp; </a>
			            </c:otherwise>
			        </c:choose>
			    </c:forEach>
			    <c:choose>
				   	 ...
				   	 <c:otherwise>
				 	    <a class="paging-link-ko" href="/free/free_list_view?category=${category}&page=${paging.page+1}&recommendCount=${recommendCount}">[다음]</a>
				   	 </c:otherwise>
		   		 </c:choose>
			</div>
		</div>
